...
(function () {
  ...
  if (full_html) {
    ...
    if (job.html.search(emailRegex) > -1)
      job.source_apply_email = job.html.match(emailRegex)[0].trim();
    job.html = job.html.replace(
      /[\w|.-]+@[\w|-]+(\.[\w-]+){1,4}|https?:\/\/\S+|loading.?.?.?/gi,
      ''
    );
  ...
  window.location.href =
    'https://thevine.wshgroup.co.uk/portal.php?site=applicantPortal&page=vacanciesPortico&filter=LEAD';
  jobs.push(job);
  out['jobs'] = jobs;
