...
(function () {
    var out = {};
    var html_jobs = document.querySelectorAll("div.jv-wrapper div.jv-job-item");
    var jobs = []; for (var x in html_jobs) {
        if (typeof html_jobs[x] == "function") continue;
        if (typeof html_jobs[x] == "number") continue;
        var job = {};
        var elem = html_jobs[x];
        job.title = elem.querySelector("div.jv-job-list-name a").textContent.trim();
        job.url = elem.querySelector("div.jv-job-list-name a").href.trim() + "?__jvst=Job Board&jvsd=talent";
        job.location = elem.querySelector("div.jv-job-list-location").textContent.trim();
        job.reqid = job.title.split(' - ')[1].replace('Req ID ', '');
...
(function () {//Dec-2020
    ...
    for (const a of full_html.querySelectorAll('p')) {
        if (a.textContent.search(/EEO, Veterans & Disabled /i) > -1) {
            a.remove();
        }
    }
