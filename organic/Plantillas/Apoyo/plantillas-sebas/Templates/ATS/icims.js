...
(function () {
    ...
    var html_jobs = iframeDocument.querySelectorAll('div[class*="iCIMS_JobsTable"] > div[class="row"]');
    var jobs = [];
    for (var x in html_jobs) {
        ...
        for (var a of elem.querySelectorAll('div[class*="header"] > span:nth-child(1)')) {
            ...
            if (a.textContent.search(/Posted Date/ig) > -1) {
                job.dateposted_raw = a.parentElement.querySelector('span:nth-child(2)').textContent.trim();
        ...
        for (var a of elem.querySelectorAll('div[class*="additionalFields"] dl')) {
            ...
            if (a.querySelector('dt').textContent.search(/Tipo de Posición/ig) > -1) {
        ...
        success: function (result) {
            ...
            for (var elem of html_jobs) {
                ...
                for (var a of elem.querySelectorAll('div[class*="header"] > span:nth-child(1)')) {
                    ...
                    if (a.textContent.search(/Posted Date/ig) > -1) {
                        job.dateposted_raw = a.parentElement.querySelector('span:nth-child(2)').getAttribute('title').split(' ').shift().trim();
                        //job.dateposted_raw = dateAgo(job.dateposted_raw, " ", 0, 1);
                    }
                }
                if (!job.location) {
                ...
                for (var a of elem.querySelectorAll('div[class*="additionalFields"] dl')) {
                    ...
                    if (a.querySelector('dt').textContent.search(/Tipo de Posición|Type/ig) > -1) {
                        job.source_jobtype = a.querySelector('dd').textContent.trim();
                    }
                    if (a.querySelector('dt').textContent.search(/Fecha de cierre de la postulación|Closed Date/ig) > -1) {
                        job.dateclosed_raw = a.querySelector('dd').textContent.trim();
